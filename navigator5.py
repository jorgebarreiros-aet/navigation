...
class GoToBeaconOdom(Node):
    def __init__(self):
        ...
        self.create_subscription(Pose, "/beacon/pose", self.on_beacon_pose, 1)
    ...
    def control_step(self) -> None:
        ...
        if self.beacon_pose is None or self.odom_time is None or self.beacon_time is None:
            return

        now = self.get_clock().now()
        age_beacon = (now - self.beacon_time).nanoseconds * 1e-9

        if age_beacon > self.min_data_age_sec:
            self.publish_stop()
            self.get_logger().warn("Data stale -> stopping.")
            return
        ...
        # Target position 
        tx = self.beacon_pose.position.x
        ty = self.beacon_pose.position.y
